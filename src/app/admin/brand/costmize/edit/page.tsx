...
const Costmizecatgorey = () => {
  ...
  const [formdata, setFormdata] = useState({
   ...
   const name="category"
            const url="/admin/brand/costmize"
...
if (!formdata.title || !formdata.subtitle) {
  ...
  return;
    ...
    try {
      ...
      router.push("/admin/brand/costmize");
    } catch (error: unknown) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("costmize added successfully:", data);
    } catch (error) {
      console.error("Error submitting the form:", error);
    }
  };
  return (
    <Costmizecomp name={name} handleSubmit={handleSubmit }handleUpdate={handleUpdate } formdata={formdata} handleChange={handleChange} error={error} url={url}/>
  );
};
